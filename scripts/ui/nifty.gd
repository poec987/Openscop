	...
	if visible:
		if !loaded:
			if get_tree().get_root().get_child(4).find_child("level_root").find_child("visual_mesh"):
				texture_data = get_tree().get_root().get_child(4).find_child("level_root").find_child("visual_mesh").get_child(0).get_surface_override_material(0).get_shader_parameter("albedoTex")
